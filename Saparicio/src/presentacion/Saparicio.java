...
public class Saparicio {
        ...
        if (val == 0) {
            ...
            switch (token.getAtributo()) {
                ...
                case Token.ELIMINARPRODUCTO: {
                    /*
                    if (user.tipoUsuarioxEmail(destinatario) == 1) {
                        eliminarProducto(analex, destinatario);
                    } else {
                        ClienteSMTP.sendMail(destinatario, "Permisos Insuficientes!", Constante.PermisosInsuficientes);
                    }
                    */
                    eliminarProducto(analex, destinatario);
                }
                ...
                case Token.ESTADISTICASCLIENTESFIELES: {
                    estadisticasCientesFieles(analex, destinatario);
                }
                break;
    ...
    private void registrarProducto(Analex analex, String correoDest) {
        ...
        try {
            ...
            mimemailer.sendHtmlEmail(correoDest, "Registrar Producto", Constante.IngresoPositivoR + "\n\n" + Utils.dibujarTablawithHTMLwithoutOpciones(producto.mostrarProductos()));

        } catch (Exception e) {
            ClienteSMTP.sendMail(correoDest, "Registrar Producto", Constante.IngresoErrorR + "\n" + "Mensaje enviado: " + analex.M.texto);
    ...
    private void registrarUsuario(Analex analex, String correoDest) {
        ...
        try {
            ...
            usuario.registrarUsuario(ci, nombres, paterno, materno, fechaNac, 
                    telefono, email, direccion, user, password, idRol);
    ...
    private void modificarUsuario(Analex analex, String correoDest) {
        ...
        if (usuario.existeUsuario(id)) {
            try {
                ...
                usuario.modificarUsuario(id, ci, nombres, paterno, 
                        materno, fechaNac, telefono, email, direccion,
                        user, password, estado, idRol);
                //clienteNegocio.modificarCliente(id,nombre,apellido,telefono, direccion, email);
                // ClienteSMTP.sendMail(correoDest, "Modificar Cliente", Constante.IngresoPositivoM);
                mimeMail mimemailer = new mimeMail();
    ...
    private void registrarCliente(Analex analex, String correoDest) {
        ...
        try {
            ...
            cliente.registrarCliente(ci, nombres, paterno, materno, fechaNac, 
    ...
    private void modificarCliente(Analex analex, String correoDest) {
        ...
        if (cliente.existeCliente(id)) {
            try {
                ...
                cliente.modificarCliente(id, ci, nombres, paterno, 
                        materno, fechaNac, telefono, email, direccion, estado);
                //clienteNegocio.modificarCliente(id,nombre,apellido,telefono, direccion, email);
                // ClienteSMTP.sendMail(correoDest, "Modificar Cliente", Constante.IngresoPositivoM);
                mimeMail mimemailer = new mimeMail();
    ...
    private void realizarVenta(Analex analex, String correoDest) {
        ...
        try {
            ...
            */
            ...
            venta.registrarVenta(total, estado, numcomprobante, idCliente, idUsuario, detalleVenta);
    ...
    private void obtenerVentaYDetalles(Analex analex, String destinatario) {
        ...
        if (id != 0 && venta.existeVenta(id)) {
            ...
            String mensaje = Utils.dibujarTablawithHTMLwithoutOpcionesVenta(venta.mostrarVentaId(id), 
    ...
    private void obtenerIngresoYDetalles(Analex analex, String destinatario) {
        ...
        if (id != 0 && ingreso.existeVenta(id)) {
            ...
            String mensaje = Utils.dibujarTablawithHTMLwithoutOpcionesIngreso(ingreso.mostrarIngresoId(id), 
    ...
    private void estadisticasCientesFieles(Analex analex, String correoDest) {
        // Obtengo el Siguiente token
        analex.Avanzar();
        Token token = analex.Preanalisis();
